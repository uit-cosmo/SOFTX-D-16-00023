! Outout the 48 time steps results into netcdf
      ...
      subroutine albedo_output (input_albedo_param)
  ...
  !     This is the name of the data file we will create.
        character*(*) FILE_NAME
        parameter (FILE_NAME = '../output/albedo_output.nc')
        integer ncid
        integer NDIMS 
        parameter (NDIMS = 4)
  ...
  !     We will create the temperature field.
        ...
        parameter (Albedo_Name='co-albedo')
  ...
  !      real temp_out(NLONS, NLATS,NRECS)
        real albedo_out(NLONS,NLATS,NLVS,NRECS),Pcoalbedo2(NLONS,NLATS)
  ...
  !     Error handling.
      ...
      do teller = 2, NYEARS
          do ts = 1, 48
              ...
              BINfile='../output/al'//tsnum//'.'//tellerstring//'.bin'
              open (unit=106, file=BINfile, status='old')
              read (106,*) Pcoalbedo2
              close (106,status='delete')
              tellerto = tellerto + 1
              albedo_out(:,:,NLVS,tellerto) = Pcoalbedo2(:,:)
        end do
      end do
  ...
  !     Create the file. 
        if (input_albedo_param == 0) then
              retval = nf_create('../output/albedo-output_no_albedo.nc', nf_clobber, ncid) 
        else if (input_albedo_param == 1) then 
              retval = nf_create('../output/albedo-output_albedo.nc', nf_clobber, ncid) 
        end if 
  ...
  !     sure your data are really written to disk.
        ...
        print *,NRECS, ' time steps co-albedo results stored successfully in ', FILE_NAME, '!'
        write(2,*), NRECS,' time steps co-albedo results stored successfully in ', FILE_NAME, '!'      
        end
